...
// =============================================================
//
// **VERBATIM PORT** of the onboarding tool's PIN module at
// `src/lib/onboarding/pin.ts`. The hash format and scrypt
// parameters MUST stay in lockstep across both apps so a PIN
// rotated from either side remains verifiable from the other.
//
// If you need to change anything here, change the onboarding
// side first, then mirror it here in the same commit. Drift
// between the two will silently invalidate previously-issued
// PINs.
//
...
//
// Phase 1 status: this module is written and unit-callable but
// NOT wired to any UI yet. Phase 12 of the spec (Regenerate PIN
// modal) will be the first consumer.
...
// Tuning notes — keep in sync with onboarding/src/lib/onboarding/pin.ts.
...
// Lockout policy
// =============================================================
// Pure functions — no IO, no DB writes. Callers read the current
// session state, compute the next state via these helpers, and
// write it back. Mirrors the onboarding side exactly.
