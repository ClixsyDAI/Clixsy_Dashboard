...
- **Underlying Google Cloud project**: project number `170670946909`. Owned by the Google account that originally set up the onboarding repo (likely `johanlcilliers@gmail.com`); `johan@clixsy.com` does **not** have IAM access as of 2026-05-31. Confirmed by attempting to load `https://console.cloud.google.com/auth/overview?project=170670946909` while signed in as `johan@clixsy.com` and receiving an access-denied page with missing permissions: `clientauthconfig.clients.list`, `oauthconfig.verification.get`, `resourcemanager.projects.get`, `serviceusage.quotas.get`.
- **Client ID**: `170670946909-bc9mg87h2q5a957qchc14vlcaekscfm4.apps.googleusercontent.com`
- **Consent screen User Type**: **External** (inferred — not directly verified). The clixsy.com Workspace account `johan@clixsy.com` cannot read this project, which means the project is not inside the clixsy.com Workspace. Google's "Internal" User Type requires the project to live inside the consuming user's Workspace, so the consent screen on a non-clixsy.com-Workspace project cannot be Internal. Treat External as the working assumption.
- **Hosted-domain restriction (`hd=clixsy.com`)**: enforced at the application layer by Phase 1 PR B's `signInWithOAuth({ provider: 'google', options: { queryParams: { hd: 'clixsy.com' } } })` call. **This is the primary enforcement layer** for "only `@clixsy.com` accounts may sign in" — because the consent screen is External (per above), the consent-screen layer does NOT restrict by domain. Google honors the `hd` parameter server-side per the OIDC spec, so a non-clixsy.com account is rejected during the OAuth handshake itself, before the callback ever fires. The `app_users` table lookup in `/admin/auth/callback` is a second layer that catches anyone whose email isn't on the access list.
